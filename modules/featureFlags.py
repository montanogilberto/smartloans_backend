...
def is_non_custodial_funding_enabled(company_id=None, client_id=None) -> bool:
    ...
    Service config / .env, no code change or deploy needed to flip it:
        ...
        NON_CUSTODIAL_FUNDING_TEST_COMPANY_IDS=1008      — comma-separated
        NON_CUSTODIAL_FUNDING_TEST_CLIENT_IDS=2165,2167  — comma-separated
    ...
    if os.getenv("ENABLE_NON_CUSTODIAL_FUNDING_FLOW", "false").strip().lower() == "true":
        return True
    if company_id is not None and int(company_id) in _id_set("NON_CUSTODIAL_FUNDING_TEST_COMPANY_IDS"):
        return True
    if client_id is not None and int(client_id) in _id_set("NON_CUSTODIAL_FUNDING_TEST_CLIENT_IDS"):
        return True
    return False
